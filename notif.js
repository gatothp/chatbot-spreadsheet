...
async function processReports() {
    ...
    const msgAdd_ind = `\n\nJangan lupa mengisi habituasi Ramadhan utk individu di https://s.id/5buah 🙏`;
    const msgAdd_grp = `\n\nJangan lupa mengisi habituasi Ramadhan utk grup di https://s.id/5buah 🙏`;
    ...
    for (const [id, waNumber] of data) {
        ...
        const msg = hasSent
            ? `Siip 👍, kegiatan harian untuk periode ${startDateSt} - ${endDateSt} sudah dilaporkan.` + msgAdd_ind
            : `Kegiatan harian ${id} untuk periode ${startDateSt} - ${endDateSt} belum dilaporkan.\n\nYuk lapor segera di s.id/5harian 🙏` + msgAdd_ind;

        process.stdout.write(`${id} ${hasSent ? '✅' : '🔴'}`);
        await sendWA(waNumber, msg);
    }

    // Group Reports
    const grups = data.filter(item => item[3] == 1).map(item => [item[1], item[2]]);
    for (const [waNumber, id] of grups) {
        ...
        const msg = hasSent
            ? `Siip 👍, rapat ${id} untuk periode ${startDateSt} - ${endDateSt} sudah dilaporkan.` + msgAdd_grp
            : `Rapat ${id} untuk periode ${startDateSt} - ${endDateSt} belum dilaporkan.\n\nYuk lapor segera di s.id/5bertemu 🙏` + msgAdd_grp;

        process.stdout.write(`Grup ${id} ${hasSent ? '🍏' : '🍎'}`);
        await sendWA(waNumber, msg);
    }
